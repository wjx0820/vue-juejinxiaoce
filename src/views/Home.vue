        ...
        <li class="item" v-for="item in bookData" :key="item.id">
          <router-link :to="{path:`/detail/${item.id}`}">
            ...
            <div class="con">
              <h4>{{item.title}}</h4>
              <div class="author">
                <img round :src="item.userData.avatarLarge" alt />
                <h6>{{item.userData.username}}</h6>
              </div>
              <p>{{item.lastSectionCount}}小节 ▪ {{item.buyCount}}人购买</p>
            </div>
...
<script>
//类别数据：对于不需要更新或者更新后不需要通知组件渲染的数据，我们不让其数据劫持=>Object.freeze不会给types内层的数据绑定getter,setter，但types本身会被劫持
let types = [
  {
    value: "",
    name: "全部"
 ...
 *   【服务器骨架屏】
 *        1. 服务器渲染 SSR （项目是半前后端分离开发：首屏服务器渲染，其余屏幕在首屏加载完成后，由客户端渲染）
